// ... (all the existing calculator logic from the previous version) ...
// Ensure this script is loaded AFTER the HTML body so elements exist.
...
function appendOperator(op) {
    ...
    if (calculationDone) {
        ...
        calculationDone = false;
    } else if (currentInput !== "") { // Ensure currentInput is not empty before parsing
        firstOperand = parseFloat(currentInput);
    }
...
function percentage() {
    ...
    if (operator && firstOperand !== null && !shouldResetDisplay) {
        ...
        switch (operator) {
            ...
            case '-': result = firstOperand - percOfFirst; break;
            case '*': result = firstOperand * (currentValue / 100); /* Direct percent of first for mult */ break; 
            case '/': 
                if (currentValue === 0) { currentInput = "Error"; updateDisplay(); resetCalculatorStateAfterError(); return; }
                result = firstOperand / (currentValue / 100); /* Divide by percent value */ break;
        }
...
document.addEventListener('keydown', function(event) {
    ...
    event.preventDefault(); // More aggressive preventDefault for better control
...
// --- NEW: Interactive Button Background Effect ---
document.querySelectorAll('.interactive-bg').forEach(button => {
    ...
    button.addEventListener('mousemove', function(e) {
        ...
        // Determine highlight color based on original button background lightness
        // This is a simple heuristic; more sophisticated methods exist
        const originalBg = button.dataset.originalBg;
        let highlightColor = getComputedStyle(document.documentElement).getPropertyValue('--interactive-highlight').trim();

        // A simple check if originalBg is dark (this needs refinement for robust check)
        // For simplicity, we assume .btn-num and .btn-util are lighter
        // if (originalBg.startsWith('rgb') && parseInt(originalBg.split(',')[0].replace('rgb(','')) < 128) {
        //     highlightColor = getComputedStyle(document.documentElement).getPropertyValue('--interactive-highlight-darker').trim();
        // }


        button.style.backgroundImage = `radial-gradient(circle at ${x}px ${y}px, ${highlightColor} 0%, transparent 80%)`;
        button.style.backgroundColor = originalBg; // Keep original bg color underneath
    });

    button.addEventListener('mouseleave', function() {
        button.style.backgroundImage = 'none'; // Remove the radial gradient
        button.style.backgroundColor = button.dataset.originalBg; // Restore original bg explicitly
    });
});
